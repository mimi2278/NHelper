...
class NovelHelperPlugin extends obsidian_1.Plugin {
    ...
    async activateView() {
        ...
        if (!leaf) {
            ...
            if (leaf)
                await leaf.setViewState({ type: VIEW_TYPE_NOVEL_HELPER, active: true });
        }
        if (leaf)
...
class NovelHelperView extends obsidian_1.ItemView {
    ...
    buildRequest(preset, fileContents, userMessage) {
...
class ReferencePickerModal extends obsidian_1.Modal {
    ...
    renderNode(container, node, depth) {
        if (node instanceof obsidian_1.TFile) {
            ...
            cb.onchange = () => {
                if (cb.checked)
                    this.tempSelected.add(node.path);
                else
                    this.tempSelected.delete(node.path);
            };
        ...
        if (node instanceof obsidian_1.TFolder) {
            ...
            cb.onchange = () => {
                for (const path of childMd) {
                    if (cb.checked)
                        this.tempSelected.add(path);
                    else
                        this.tempSelected.delete(path);
                }
                this.onOpen();
            };
